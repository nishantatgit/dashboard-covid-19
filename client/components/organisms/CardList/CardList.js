...
class CardList extends React.Component{
  ...
  render(){
    return (
      <React.Fragment>
        <div className="card-menu">{this.props.data.map( (hotel, idx, hotels) => <Card lastClass={idx === hotels.length-1} key={hotel.Name} name={hotel.Name} city={hotel.City} rating={hotel.Rating}></Card>)}
        </div>
  ...
  componentDidMount(){
    ...
    setIntersectionObserver({rootMargin : '10px'}, 'bottom-card-menu', onTargetEnter ); 
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    getCardListData: (hotelDisplayIndex) => dispatch({ type : FETCH_CARD_LIST_DATA_FROM_API , payload : hotelDisplayIndex}),
...
export default connect(mapStateToProps,mapDispatchToProps)(StyledCardList);
